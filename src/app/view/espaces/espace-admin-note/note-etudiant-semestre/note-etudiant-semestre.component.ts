...
export class NoteEtudiantSemestreComponent implements OnInit {
  ...
  constructor(private router:Router,private noteEtudiantSemestreService:NoteEtudiantSemestreService,private anneeUniversitaireService:AnneeUniversitaireService) {
  ...
  downloadExcel() {
    ...
    worksheet.columns = [
      { header: 'Etudiant', key: 'Etudiant', width: 17 },
      { header: 'note', key: 'note', width: 23 },
      { header: 'Etat de Validation', key: 'EtatdeValidation', width: 23}
    ];

    this.myNotesSemestre.forEach(e => {
      worksheet.addRow({
        Etudiant: e.etudiant.nom+e.etudiant.prenom,
        note: e.noteSemestre,
        EtatdeValidation:e.etatValidation.libelle
      }, 'n');
    });

    workbook.xlsx.writeBuffer().then((data) => {
      const blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      fs.saveAs(blob, 'PV_etudiants_'+this.input1+this.input2+'.xlsx');
    });
  }
}
